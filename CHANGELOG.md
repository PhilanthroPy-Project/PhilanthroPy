...
- **mypy ratchet started.** `py.typed` ships in the wheel, so a user's type
  ...
  fails CI rather than enlarging the backlog. `experimental` is now annotated and listed
  too. `model_selection` (open PR), `cli`, `models` and `preprocessing`
  remain, tracked in #166.
