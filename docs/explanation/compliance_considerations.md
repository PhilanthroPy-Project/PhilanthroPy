...
!!! warning "`allow_negative_days=True` warns at fit time for a reason"
    Fitting with `allow_negative_days=True` emits a `UserWarning`. Soliciting a patient during an active care episode raises ethical concerns well beyond HIPAA. The Association of Fundraising Professionals (AFP) *Donor Bill of Rights* and the Association for Healthcare Philanthropy (AHP) guidelines on grateful-patient programs both counsel care around the timing of clinician-referred or care-adjacent solicitation. Enable this flag only for legitimate retrospective analysis, and confirm the use with your donor-relations policy and Privacy Officer first.
...
**You are responsible** for ensuring that the data you feed the library is lawful to use for fundraising, appropriately de-identified or authorized, and scoped to the minimum necessary. You are also responsible for involving your Privacy Officer and counsel before production.
